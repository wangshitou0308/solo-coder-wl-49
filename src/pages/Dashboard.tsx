...
const stageColors: Record<BinStage, string> = {
...
export default function Dashboard() {
  ...
  const maxWeight = stats
    ? Math.max(...stats.topResidents.map((r) => r.totalWeight), 1)
    : 1;

  return (
    <div className="space-y-6">
      <h1 className="text-xl font-serif font-bold text-[#1B4332]">数据看板</h1>
          ...
          {(stats?.topResidents ?? []).map((r) => (
            <div key={r.residentId} className="flex items-center gap-3">
              <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                r.rank <= 3 ? 'bg-compost-green text-white' : 'bg-gray-100 text-gray-500'
              }`}>
                {r.rank}
              </span>
              ...
              <div className="flex-1 bg-gray-100 rounded-full h-5 overflow-hidden">
                <div
                  ...
                  style={{ width: `${(r.totalWeight / maxWeight) * 100}%` }}
                />
              </div>
              <span className="text-sm text-gray-500 w-16 text-right">{r.totalWeight} kg</span>
            </div>
          ))}
